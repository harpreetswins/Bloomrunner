...
namespace CRM.Data.GenericRepository
{
    public class GenericRepository
    {
        ...
        //     }
        // }
         protected async Task<T> ExecuteAll<T>(string sql, object parameters = null)
        {
            using (var connection = CreateConnection())
            {
                var QueryResponse = await connection.QueryFirstOrDefaultAsync<T>(sql: sql, param: parameters, commandType: CommandType.StoredProcedure);
                return QueryResponse;
            }
        }         
    }
